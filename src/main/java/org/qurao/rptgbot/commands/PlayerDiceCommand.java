...
public class PlayerDiceCommand implements ICommand{
				...
				if(usersStorage.hasProfile(player)) {
					...
					if(type.equals("str")) {
						...
						bonus = stats.getStrength() 
								+ profile.getBonusFromItemsAndFeatures().getStrength();
					}else if(type.equals("int")){
						typeName = "Интеллект";
						bonus = stats.getIntelligence() 
								+ profile.getBonusFromItemsAndFeatures().getIntelligence();
						...
						bonus = stats.getCharisma() 
								+ profile.getBonusFromItemsAndFeatures().getCharisma();
					}else if(type.equals("agi")) {
						typeName = "Ловкость";
						bonus = stats.getAgility() 
								+ profile.getBonusFromItemsAndFeatures().getAgility();
					}else {
						incorrect = true;
					}
					if(incorrect) {
